...
@section('content')
            ...
            <div class="row">
                <div class="col-sm-12 d-flex">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item dana">تعریف طرح</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <form action="{{ route('subscription-plan.store') }}" method="POST" class="row">
        @csrf
                    ...
                    <div class="card-wrapper border rounded-3 row">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="form-control"
                                      placeholder="مثلا دسترسی به تمامی امکانات اصلی برای یک ماه...">{{ old('description') }}</textarea>
                ...
                <div class="card-body">
                    <div class="card-wrapper border rounded-3 row">
                        <div class="mb-3" x-data="{
                            ...
                            <input type="text" class="form-control" value="{{ old('price') }}" x-ref="price" name="price" id="price" placeholder="مبلغ را وارد کنید...">
                            <x-input-error :messages="$errors->get('price')" class="mt-2"/>
                        </div>
                        <div x-data="{
                            isLifetime: @js(old('is_lifetime')) ?? false
                        }">
                            <div class="media">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="type" id="type">
                                @foreach(PlanType::toSelectOptions() as $type)
                                    <option value="{{ $type['value'] }}" @selected(old('type', 'both') == $type['value'])>{{ $type['label'] }}</option>
                                @endforeach
                            </select>
